...
**Resolve the absolute project path from the filesystem — do NOT guess or construct it:**

```bash
realpath projects/[client]/[project]/
```

Store this as the canonical `project_path`. This is the value that goes into the implementation spec frontmatter and tickets.yaml. The path MUST match where the project actually lives on disk within the project-creator directory structure.
